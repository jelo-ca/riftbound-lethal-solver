...
|---|---|---|
| `PlayUnit(card_id, target_zone, rune_payment, trigger_params=())` | Move card from hand to Base **or** to a battlefield the controller already controls (rule 355.7/355.8 — both are valid targets, confirmed) **or** an open battlefield if the card grants that exception (`CardDef.can_play_to_open_battlefield`, e.g. Sneaky Deckhand) as a new `UnitInstance`, entering exhausted (rule 143.4.a). Establishes control (a Conquer-eligible event) when the target was open. If the card has a registered "when you play me" trigger (`abilities.UNIT_PLAY_TRIGGERS`, e.g. Blitzcrank), `trigger_params` carries the chosen trigger choice — see "Unit play triggers" below | Chosen `rune_payment` (which specific runes get spent) covers Energy (any domain) + Power (domain-matched) cost from `RunePool.available`; card is a Unit type; a non-empty `trigger_params` is additionally validated by the registered trigger's own legality check |
...
**`rune_payment` is itself a choice**, but a narrower one than first documented. **Corrected 2026-09-14:** one rune yields Energy (Exhaust) **and** Power of its own domain (Recycle) — both, independently, in either order. The two capacities never compete, so paying the Energy portion never constrains the Power portion, and *which* runes are Exhausted is unobservable (Energy ignores domain). The remaining genuine choice is on the **Recycle** side: a domain-free cost (today only `[Deflect]`'s rainbow tax) consumes a *specific* domain's Recycle capacity, so picking Fury over Order there can strand a later Order spell. Those splits are enumerated; the Energy side takes one representative, which is exact rather than a simplification.
...
**Activated abilities and unit play triggers use the same pattern.** `abilities.ABILITY_EFFECTS` (keyed by the source unit's `card_id`) backs `ActivateAbility` — first entry Caitlyn - Patrolling's exhaust-cost damage ability, needed for puzzle 5. `abilities.UNIT_PLAY_TRIGGERS` backs `PlayUnit.trigger_params` for "when you play me" effects — first entry Blitzcrank - Impassive's "you may move an enemy unit to here," needed for puzzle 6. Unlike the other two registries, a unit-play-trigger's effect can return **multiple** outcomes (`list[GameState]`, via `combat.enumerate_combat_outcomes`) since moving an *enemy* unit onto ground we hold causes combat with the opponent as Attacker — see `09-combat-resolution.md`. `search.py`'s `_and_or_search` (generalized out of what was originally `ResolveCombat`-specific) handles the AND-node for any action whose resolution can branch this way, not just `ResolveCombat` — built generically since more "when played" triggers are expected.
