...
3. **Dual-cost rune payment (correction, then RE-corrected 2026-09-14)** — this entry previously read "a rune produces Energy **or** Power, never both (rule 164.2.b)," treating the blog source that said "same rune pays both" as wrong. **The blog was right.** A rune gives 1 Energy (Exhaust) *and* 1 Power of its own domain (Recycle), independently and in either order — Recycling a ready rune leaves a "floating rune" that can still be Exhausted. Confirmed against card text: "ready 4 friendly runes", "Recycle me to ready your runes", and ~12 cards specifying "channel 1 rune **exhausted**", none of which parse if Exhausting consumes a rune. The old reading roughly halved modelled resources. `RunePool` now carries `energy_spent`/`power_spent` trackers instead of deleting runes.
