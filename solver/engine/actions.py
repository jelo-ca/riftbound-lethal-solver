...
@dataclass(frozen=True)
class ResolveConquerTrigger:
    """Resolves a pending choice-bearing conquer trigger recorded via
    GameState.pending_conquer_choice (engine/conquer.py) — the ONLY legal
    action while one is pending, same gating shape as
    ResolveAttackTrigger/ShowdownState.attack_trigger_resolved. `params`
    is opaque, effect-specific, generated by whichever registry
    (conquer.CONQUER_TRIGGERS_WITH_CHOICE for a unit-keyed trigger,
    conquer.BATTLEFIELD_CONQUER_TRIGGERS for a battlefield-keyed one)
    matches state.pending_conquer_choice.kind/key. An empty tuple always
    means "decline" for an optional trigger; a mandatory one (Zaun
    Warrens) has no decline candidate, only real choices (one of which
    may itself be a no-op, e.g. discarding from an empty hand).
    """
    params: tuple
...
Action = (PlayUnit | MoveUnit | ResolveCombat | EnterShowdown | ResolveShowdown
          | ResolveAttackTrigger | ResolveConquerTrigger | PlaySpell | PlayGear | ActivateAbility)
