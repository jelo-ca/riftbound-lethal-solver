...
def test_keyword_bonus_does_not_apply_outside_combat():
    """Confirmed rule: the bonus only counts while the keyword's own
...
def test_assault_raises_the_lethal_threshold_not_just_damage_dealt():
    """Might is one stat doing both jobs. A 3-Might Assault attacker is
    effectively 4 Might while attacking, so a 3-damage defender can't
...
def test_lethal_first_enumeration_accounts_for_the_targets_effective_might():
    """A Shield defender needs 4 assigned, not 3 — the assignment
